              ...
              <div class="box mt-0 mb-lg-0">
                <div class="table-responsive">
                  <table class="table table-hover">
                    ...
                    <tr>
                        <td>{{$no ++ }}</td>
                        <td><img src="{{ asset('static/dist/img/' . $item->photo) }}" width="50px"></td>
                        <td>{{ $item->name_product }}</td>
                        <td>{{ $item->stock }}</td>
                        <td>{{$item->user->name_user }}</td>
                        <td>
                            <a href="{{ url('editproduct/'.$item->id_product) }}" class="btn btn-sm btn-primary">Edit</a>
                            <a href="{{ url('deleteproduct/'.$item->id_product) }}" class="btn btn-danger btn-sm">Delete</a>
                        </td>
                        </tr>
                    @endforeach
                    </tbody>
